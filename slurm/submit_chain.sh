...
for i in $(seq 1 "$N"); do
  ...
  if [ -z "$dep" ]; then
    id=$(sbatch --parsable --job-name="$name" --export=ALL,CFG="$CFG",RUN_ID="$RUN_ID",FINAL_CKPT="$FINAL" "$REPO/slurm/run_unit.sbatch" | cut -d';' -f1)
  else
    id=$(sbatch --parsable --dependency=afterany:$dep --job-name="$name" --export=ALL,CFG="$CFG",RUN_ID="$RUN_ID",FINAL_CKPT="$FINAL" "$REPO/slurm/run_unit.sbatch" | cut -d';' -f1)
  fi
  ids+=("$id"); dep=$id
done
echo "${ids[*]}"
