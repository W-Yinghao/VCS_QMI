...
BASELINES = {  # (stage, seed) -> reference linear/kNN for deltas
    ...
    ("P28_vcs_new_candidates", 0): {"run": "P24_vcs_cos_negdetach_seed0 (cosine, K=8, neg-detach)", "linear": 80.48, "knn": 74.62, "h_rank": 30.87},
    ...
    ("P39_vcs_recipe_ablation", 0): {"run": "P26_vcs_a5_learn_seed0 (a0=5, 2 views, 200 ep = 1x compute)", "linear": 81.90, "knn": 77.00, "h_rank": 44.99},
    ("P43_vcs_ceiling", 0): {"run": "P35_vcs_a5_800ep_seed0 (a0=5, 2 views, 800 ep; best final so far)", "linear": 85.54, "knn": 82.62, "h_rank": 88.18},
    ...
    ("P29_vcs_seedfill", 2): {"run": "seed-0 of the same config (cosK8 78.32 / cosK255 79.32)", "linear": 78.32, "knn": 72.76, "h_rank": 57.74},
...
BASE_HP = {"K": 1, "critic_hidden_dims": [512, 512], "critic_last_gain": 0.1, "critic_lr_multiplier": 1.0, "critic_weight_decay": 0.0,
           ...
           "critic_feature_source": "z", "negative_detach": False, "target_branch": "shared", "predictor": False, "projector_depth": 2, "cosine_scale_init": 1.0, "cosine_scale_fixed": False, "cosine_bias_calibrate": False, "projector_output_bn": False, "projector_kind": "mlp"}
...
def hparams(rd: Path):
    ...
    return {"K": c["pairing"]["k"], "critic_hidden_dims": c["model"]["critic"]["hidden_dims"], "critic_last_gain": c["model"]["critic"]["last_layer_xavier_gain"],
            ...
            "projector_output_bn": c["model"]["projector"]["output_batchnorm"], "projector_kind": c["model"]["projector"].get("kind", "mlp")}
...
def scan(output_root: Path, stages: set[str]):
    ...
    for rd in sorted(p for p in output_root.iterdir() if p.is_dir() and "_FAILED_attempt" not in p.name):  # archived failed attempts are listed in job_ids.json, not here
        ...
        if st.get("stage") in ("P35_vcs_a5_base", "P37_vcs_views_curve", "P39_vcs_recipe_ablation", "P43_vcs_ceiling"):
...
def main():
    ...
    ap.add_argument("--stages", default="P35_vcs_a5_base,P37_vcs_views_curve,P43_vcs_ceiling")
